import { StyleSheet, View, Text, ScrollView, RefreshControl, TouchableOpacity, Image, Alert, Modal, TextInput, ActivityIndicator, Animated, Dimensions, Linking, Platform } from 'react-native';
import { useLocalSearchParams, useRouter, Stack } from 'expo-router';
import { useEffect, useState, useCallback, useRef } from 'react';
import { getGuestByCode, getWeddingDetails, getWeddingCard, getGalleryByWedding, uploadGalleryItem, deleteGalleryItem, getAssignmentByWedding, getUserByClerkId, submitAttendeeRating, getWeddingMessagesForGuest } from '@/utils/api';
...
export default function GuestDetailScreen() {
    ...
    const loadData = async () => {
        try {
            const guestData = await getGuestByCode(code as string);
            setGuest(guestData);

            if (guestData.coupleClerkId) {
                const [weddingData, cardData] = await Promise.all([
                ...
                if (weddingData?.id) {
                    const [gallery, assignmentData, messagesData] = await Promise.all([
                        getGalleryByWedding(weddingData.id),
            ...
            Alert.alert("Error", "Could not load invitation details.");
        }
    };
        ...
        if (!result.canceled && result.assets[0].base64) {
            ...
            try {
                ...
                const itemData = {
                    ...
                    fileUrl: `data:image/jpeg;base64,${asset.base64}`,
                    fileType: 'IMAGE',
                    caption: `Shared by ${guest.firstName}`,
                };

                await uploadGalleryItem(guest.uniqueCode, wedding.id, itemData);
                Alert.alert("✨ Success", "Added to gallery!");
                loadData();
            } catch (error) {
                console.error("Upload error:", error);
                Alert.alert("Error", "Failed to upload. Try a smaller image.");
            } finally {
        ...
        Alert.alert(
            ...
            [
                { text: "Cancel", style: "cancel" },
                {
                    ...
                    onPress: async () => {
                        try {
                            await deleteGalleryItem(itemId, guest.uniqueCode);
    ...
    const openMaps = () => {
        ...
        const url = Platform.select({
            ...
            default: `https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(wedding.location)}`
        });
        Linking.openURL(url);
    };
    ...
    if (!guest) {
        return (
            <View style={styles.loadingContainer}>
                ...
                <Text style={styles.loadingText}>Preparing your invitation...</Text>
            </View>
        );
    }
    ...
    return (
        <View style={styles.container}>
            ...
            <View style={styles.editorialHeader}>
                ...
                <Text style={styles.editorialBrand}>
                    {wedding?.partnersName ? wedding.partnersName.toUpperCase() : "THE CELEBRATION"}
                </Text>
            </View>
            ...
            >
                <Animated.View style={[styles.heroSection, { opacity: fadeAnim, transform: [{ translateY: slideAnim }] }]}>
                    ...
                    <Text style={styles.heroQuote}>
                        "A journey of a thousand miles begins with a single step, and our most beautiful steps are taken together."
                    </Text>
                ...
                <Animated.View style={[styles.section, { opacity: fadeAnim, transform: [{ scale: qrScale }, { translateY: slideAnim }] }]}>
                    <View style={styles.editorialSectionHeader}>
                        <Text style={styles.editorialSectionTitle}>— ENTRY PASS</Text>
                    </View>
                    <View style={styles.qrCard}>
                        <View style={styles.qrContainer}>
                            <Image
                                source={{ uri: `https://api.qrserver.com/v1/create-qr-code/?size=200x200&data=${guest.uniqueCode}&color=000` }}
                ...
                <Animated.View style={[styles.section, { opacity: fadeAnim, transform: [{ translateY: slideAnim }] }]}>
                    <View style={styles.editorialSectionHeader}>
                        <Text style={styles.editorialSectionTitle}>— INVITATION</Text>
                    </View>
                    <TouchableOpacity
                        style={styles.invitationCard}
                        onPress={() => setShowWeddingCardModal(true)}
                        activeOpacity={0.9}
                    >
                ...
                <Animated.View style={[styles.section, { opacity: fadeAnim, transform: [{ translateY: slideAnim }] }]}>
                    <View style={styles.editorialSectionHeader}>
                        ...
                        <TouchableOpacity style={styles.editorialUpload} onPress={handleUpload} disabled={isUploading}>
                            <Text style={styles.editorialUploadText}>{isUploading ? "..." : "ADD MOMENT"}</Text>
                        </TouchableOpacity>
                    </View>
                ...
                <Animated.View style={[styles.section, { opacity: fadeAnim, transform: [{ translateY: slideAnim }] }]}>
                    <View style={styles.editorialSectionHeader}>
                        <Text style={styles.editorialSectionTitle}>— FEEDBACK</Text>
                    </View>
                    <View style={styles.ratingList}>
                            ...
                            <TouchableOpacity style={styles.editorialRatingCard} onPress={() => { setRatingType('PROTOCOL'); setShowRatingModal(true); }}>
                                <View style={styles.ratingContent}>
                                    ...
                                    <View style={styles.ratingTextGroup}>
                                        <Text style={styles.ratingTitle}>Rate Protocol</Text>
                                        <Text style={styles.ratingSubtitle}>Tell us about your arrival experience</Text>
                                    </View>
                        ...
                        <TouchableOpacity style={styles.editorialRatingCard} onPress={() => { setRatingType('WEDDING'); setShowRatingModal(true); }}>
                            <View style={styles.ratingContent}>
                                ...
                                <View style={styles.ratingTextGroup}>
                                    <Text style={styles.ratingTitle}>Rate Wedding</Text>
                                    <Text style={styles.ratingSubtitle}>How did you like the celebration?</Text>
                                </View>
                                <IconSymbol name="chevron.right" size={16} color="#d1d5db" />
                            </View>
                        </TouchableOpacity>
                        ...
                        <View style={styles.messagesContainer}>
                            {messages.slice(0, 3).map((msg) => (
                                <View key={msg.id} style={styles.editorialMessage}>
                                    ...
                                    <Text style={styles.messageTime}>THE COUPLE · {new Date(msg.createdAt).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</Text>
                                </View>
                            ))}
                        </View>
                    </Animated.View>
                    ...
                    <Animated.View style={[styles.section, { opacity: fadeAnim, transform: [{ translateY: slideAnim }] }]}>
                        ...
                        <View style={styles.detailsCard}>
                            <View style={styles.detailItem}>
                                <Text style={styles.detailLabel}>LOCATED AT</Text>
                                <Text style={styles.detailValue}>{wedding.location}</Text>
                                <TouchableOpacity onPress={openMaps} style={styles.inlineLink}><Text style={styles.inlineLinkText}>View on Maps →</Text></TouchableOpacity>
                            </View>
                            <View style={styles.detailItem}>
                                <Text style={styles.detailLabel}>HAPPENING ON</Text>
                                <Text style={styles.detailValue}>{new Date(wedding.weddingDate).toLocaleDateString('en-US', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' })}</Text>
                            </View>
                        </View>
                    </Animated.View>
                )}
                ...
                <View style={styles.modalOverlay}>
                    ...
                    <View style={styles.modalContentContainer}>
                        ...
                        <View style={styles.starRow}>
                            {[1, 2, 3, 4, 5].map(star => (
                                <TouchableOpacity key={star} onPress={() => setRatingValue(star)}>
                                    <IconSymbol name={star <= ratingValue ? "star.fill" : "star"} size={28} color={star <= ratingValue ? Colors.light.gold : "#ddd"} />
                                </TouchableOpacity>
                            ))}
                        </View>
                        ...
                        <View style={styles.modalButtons}>
                            <TouchableOpacity style={styles.cancelBtn} onPress={() => setShowRatingModal(false)}><Text style={styles.cancelText}>CANCEL</Text></TouchableOpacity>
                            <TouchableOpacity style={styles.submitBtn} onPress={handleRate} disabled={isSubmittingRating}>
                                <LinearGradient colors={[Colors.light.gold, '#b89627']} style={styles.submitGradient}><Text style={styles.submitText}>SUBMIT</Text></LinearGradient>
                            </TouchableOpacity>
                        </View>
                    </View>
                </View>
            ...
            <Modal visible={selectedImage !== null} transparent animationType="fade">
                <View style={styles.fullScreenModal}>
                    ...
                    {selectedImage && (
                        <><Image source={{ uri: selectedImage.fileUrl }} style={styles.fullScreenImage} resizeMode="contain" />
...
const styles = StyleSheet.create({
    ...
    backLink: { position: 'absolute', left: 24, top: 60 },
    ...
    cancelBtn: { flex: 1, paddingVertical: 16, alignItems: 'center', borderWidth: 1, borderColor: 'rgba(0,0,0,0.1)' },
    cancelText: { fontFamily: Fonts.Playfair.Bold, color: Colors.light.textSecondary, fontSize: 12 },
    submitBtn: { flex: 1 },
    ...
    invitationLocationLarge: { fontFamily: Fonts.Cormorant.Regular, fontStyle: 'italic', fontSize: 16, color: Colors.light.textSecondary },
